...
# 上の範囲から決めておいた数だけランダムに重複なしでノードをピックする
pick_nodes = 6000  # 任意で変更できる

if len(low_deg_neighbors) <= pick_nodes:
    sampled_nodes = list(low_deg_neighbors.items())  # 全部出力するぷゆっ
    print(f" {pick_nodes}件ほしかったけど、{len(sampled_nodes)}件しかなかった。全部出力するね！")
else:
    sampled_nodes = random.sample(list(low_deg_neighbors.items()), pick_nodes)
    print(f"{pick_nodes}件のノードをランダムに選定")

# DataFrame に変換して保存
df = pd.DataFrame(sampled_nodes, columns=["node_id", "degree"])
