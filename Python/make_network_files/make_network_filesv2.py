# 指定上位次数ノードと接続しているし低次数以下のノードとチャネルを形成したネットワークを生成します。
# さらに指定個数のノードを高次数ノードと入れ替えます
import pandas as pd
...
# ---------------------------------------------
# low_deg_neighbors の一部を高次数ノードに置き換え
# ---------------------------------------------
num_replace = 30  # 置き換えるノード数（例）

if len(low_deg_neighbors) > num_replace:
    # 置き換える対象をランダムに選択
    replace_targets = random.sample(list(low_deg_neighbors.keys()), num_replace)

    # グラフ全体から次数が高いノード順にソート
    high_degree_nodes = sorted(G.degree, key=lambda x: x[1], reverse=True)

    # すでに low_deg_neighbors に含まれていないノードのみ選ぶ
    available_high_nodes = [(n, d) for n, d in high_degree_nodes if n not in low_deg_neighbors]

    # 上位から num_replace 個を取得
    new_high_nodes = available_high_nodes[:num_replace]

    # 置き換え実行
    for old_node, (new_node, new_deg) in zip(replace_targets, new_high_nodes):
        del low_deg_neighbors[old_node]
        low_deg_neighbors[new_node] = new_deg

    print(f"low_deg_neighbors の {num_replace} ノードを高次数ノードに置き換え完了")
    print("置き換え対象（削除）:", replace_targets)
    print("置き換え先（追加）:", [n for n, _ in new_high_nodes])
else:
    print("low_deg_neighbors の数が少ないため置き換えをスキップ")
...
# CSV出力
updated_nodes_df.to_csv(os.path.join(output_dir, f"nodes_ln_{top_x}_{degree}_{len(df)}_{num_new_nodes}_{num_replace}.csv"), index=False)
updated_edges_df.to_csv(os.path.join(output_dir, f"edges_ln_{top_x}_{degree}_{len(df)}_{num_new_nodes}_{num_replace}.csv"), index=False)
updated_channels_df.to_csv(os.path.join(output_dir, f"channels_ln_{top_x}_{degree}_{len(df)}_{num_new_nodes}_{num_replace}.csv"), index=False)
