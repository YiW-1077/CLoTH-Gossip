...
function display_progress() {
    ...
    while [ "$done_simulations" -lt "$total_simulations" ]; do
        ...
        if [ "$(python3 -c "print(float('$fraction')==0.0)")" = "True" ]; then
            progress_line=$(printf "Progress: [%-50s] 0.00%%\t%d/%d\t Failed %d\t Time remaining --:--:--" "" "$done_simulations" "$total_simulations" "$failed_simulations")
        else
...
if python3 scripts/analyze_output.py "$analysis_input"; then
    if python3 scripts/summarize_attack4.py "$analysis_input"; then
        python3 scripts/plot_attack_summary.py "$analysis_input" || echo "WARN: plot_attack_summary.py failed"
    else
