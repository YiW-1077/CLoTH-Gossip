...
struct network_params {
    ...
    unsigned int enable_prt;                // 1 to enable threshold-based reconstruction
    int prt_threshold;                      // max reconstruction attempts before abort (e.g., 30)
    uint64_t prt_abort_wait_time;           // wait time before retry (milliseconds)
    unsigned int enable_rbr;                // 1 to enable reputation-based routing
    double rbr_reputation_weight;           // weight factor for reputation in routing (e.g., 10.0)
